...
def addstaff(request):
    if request.method=='POST':
        ...
        sql="select username from addstaff where username='"+us+"'"
        a=dbconnection.onerow(sql)
        if a:
            msg="Username Already given Please try new One"
            return render(request,'staff/addcustomer.html',{'msg':msg})
        else:
            ...
            dbconnection.addrow(sql1)
                # return render(request,'admin/addstaff.html',{'a':a})
...
def leaview(request):
    un=request.session['un']
    sql="select * from leave_request where username='"+un+"'"
    b=dbconnection.allrow(sql)
    return render(request,'staff/leaview.html',{'b':b})
...
def addcustomer(request):
    if request.method=='POST':
        ...
        fs=FileSystemStorage()
        fs.save('app/static/pics/'+img.name,img)
        sql="select username from addcustomer"
        a=dbconnection.allrow(sql)
        l=[]
        for i in a:
            for j in i:
